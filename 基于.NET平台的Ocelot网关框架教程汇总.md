...
```json
{
          "DownstreamPathTemplate": "/",	//下游服务配置--转到的地址
          "UpstreamPathTemplate": "/",	//上游监听路由地址
...
**上游Host**上游Host也是路由用来判断的条件之一，由客户端访问时的Host来进行区别。比如当a.jesetalk.cn/users/{userid}和b.jessetalk.cn/users/{userid}两个请求的时候可以进行区别对待。
